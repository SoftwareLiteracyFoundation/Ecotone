...
;------------------------------------------------------------------------------
  ...
  set Stage.data time:ts-load    "DailyStage_NAVD_cm_1994-6-1_2017-5-15.csv"
  set Salinity.data time:ts-load "DailySalinityFilled_1994-6-1_2017-5-15.csv"
...
;-------------------------------------------------------------------------
  ...
  ask Sawgrass_patches [
    ...
    if count plants > 0 [
      ...
      if days_dry_ > 180 [ set reason "days_dry > 180"  set death true ]
      if not death [
...
;-------------------------------------------------------------------------
  ...
  ask Spikerush_patches [
    ...
    if count plants > 0 [
      ...
      let days_wet_ min ( list 1000000
      ...
      if depth < 60 [ set reason "depth < 60 cm"  set death true ]
      if not death [
        if days_wet_ < 700 [ set reason "days_wet < 700"  set death true ]
      ]
      if not death [
        if salinity_days > salinity_max_days_ [ set reason "salinity_days"
                                                set death true ]
...
;-------------------------------------------------------------------------
  ...
  ask Spikerush_patches [
    ...
    set salinity_threshold   5
    set days_wet           800  ; init hydroperiod to 800
  ]
...
;-------------------------------------------------------------------------
  ...
  set green_list  [ "Black Mangrove" "Red Mangrove" "White Mangrove" ]
  set blue_list   [ "Fan Palm" "Swamp Bay" "Arrowhead" "Sawgrass"
                    "Spikerush" "Black Rush" "Muhly Grass" "Beakrush"
                    "Cattail" "Morning Glory" ]
  set yellow_list [ "Buttonwood" "Buttonbush" "Seaside Oxeye" "Paurotis Palm"
                    "Saltwort" "Sea Grape" ]
  set red_list    [ "Mahogany" "Hardwood Hammock" "Swamp Woodland"
                    "Hardwood Woodland" ]
...
;-------------------------------------------------------------------------
  gis:load-coordinate-system  "R2_GIS/R2_Veg_subset.prj"
  set VegMap gis:load-dataset "R2_GIS/R2_Veg_subset.shp"
  gis:set-world-envelope gis:envelope-of VegMap

  set GaugeZones gis:load-dataset "R2_GIS/Gauge_zones.shp"
...
"" ""
PENS
"LM" 1.0 0 -2674135 true "" "plot Stage \"LM\""
"E146" 1.0 0 -8630108 true "" "plot Stage \"E146\""
"EDEN_14_182" 1.0 0 -6459832 true "" "plot Stage \"Cell_14_182\""
...
"" ""
PENS
"LM" 1.0 0 -2674135 true "" "plot Salinity.psu \"LM\""
"TR" 1.0 0 -13345367 true "" "plot Salinity.psu \"TR\""
